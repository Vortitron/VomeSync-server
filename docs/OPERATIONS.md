...
- **Public directory catalogue** (staff-owned illustrated switches):
	...
	- `vomesync-catalogue-observe.timer` runs `node catalogue/cli.js observe` every five minutes. Live sources (bridges, sittings, Wikidata offices, NOAA, USGS, TfL, Launch Library, GDACS, val.se, IsUp status pages) update themselves; a short fetch failure keeps the last good state, then `staleAfterHours` (default 24, shorter on bridges, Tube, launches, IsUp and Commons divisions) forces OFF. Calendar holidays are computed in the same pass.
